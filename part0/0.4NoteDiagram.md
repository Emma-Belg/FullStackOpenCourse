...
sequenceDiagram
    ...
    deactivate user
    activate browser
    Note right of browser: The browser sends the text the user typed into field
    browser->>server: POST https://studies.cs.helsinki.fi/exampleapp.herokuapp.com/new_note
    activate server
    Note left of server: The server responds with HTTP status code 302 - a URL redirect
